...
// TableType: the storage of table, SingleTable by default, and PackedTable to enable semi-sorting 
...
class CuckooFilter
{
    ...
    {
        std::string hashed_key = HashUtil::SHA1Hash((const char*)&item, sizeof(item));
...
public:
    ...
    explicit CuckooFilter(const size_t memory_overhead) : num_items_(0)
    {
        victim_.used = false;
        table_ = new TableType<bits_per_item>(lowerpower2(memory_overhead));
    }
    ...
    {
        table_->writeFingerprint();
    }
    void readFingerprint()
    {
        table_->readFingerprint();
    }

    // Add an item to the filter.
    Status Add(const ItemType& item);
    //add an item to the filter. workaround for char arrays. 
    Status Add(const uint64_t hv);

    // Report if the item is inserted, with false positive rate.
    Status Contain(const ItemType& item) const;
    // Workaround for contain to be used in strings
    Status Contain(const uint64_t hv) const;

    // Delete an key from the filter
    Status Delete(const ItemType& item);

    // Compare Cuckoo Filter
    void CompareCF(CuckooFilter* cf2);
...
void CuckooFilter<ItemType, bits_per_item, TableType>::CompareCF(CuckooFilter* cf2)
{
    this->table_->CompareSingleTable(cf2->table_);
}

template <typename ItemType, size_t bits_per_item, 
    template<size_t> class TableType>
Status CuckooFilter<ItemType, bits_per_item, TableType>::Add(const ItemType& item)
{
    size_t i;
    uint32_t tag;
...
//WORKAROUND for char[] situation. The index and hash will be passed as params into the funciton. 
template <typename ItemType, size_t bits_per_item, 
    template<size_t> class TableType>
Status CuckooFilter<ItemType, bits_per_item, TableType>::Add(const uint64_t hv)
{
    size_t index = IndexHash((uint32_t) (hv >> 32));
    uint32_t tag = TagHash((uint32_t) (hv & 0xFFFFFFFF));
...
template <typename ItemType, size_t bits_per_item, 
    template<size_t> class TableType>
Status CuckooFilter<ItemType, bits_per_item, TableType>::AddImpl(const size_t i, const uint32_t tag)
{
    size_t curindex = i;
    uint32_t curtag = tag;
    uint32_t oldtag;
...
template <typename ItemType, size_t bits_per_item, 
    template<size_t> class TableType>
Status CuckooFilter<ItemType, bits_per_item, TableType>::Contain(const ItemType& key) const
{
    bool found = false;
    size_t i1, i2;
    uint32_t tag;
...
template <typename ItemType, size_t bits_per_item, 
    template<size_t> class TableType>
Status CuckooFilter<ItemType, bits_per_item, TableType>::Contain(const uint64_t hv) const
{
    bool found = false;
    size_t  i2;

    size_t i1 = IndexHash((uint32_t) (hv >>32));
    uint32_t tag = TagHash((uint32_t) (hv & 0xFFFFFFFF));
    ...
    if (found || table_->FindTagInBucket(i1,tag))
    {
        return Ok;
    }
    else if (found || table_->FindTagInBucket(i2,tag))
    {
        return Ok;
    }
    /*
...
template <typename ItemType, size_t bits_per_item, 
    template<size_t> class TableType>
Status CuckooFilter<ItemType, bits_per_item, TableType>::Delete(const ItemType& key)
{
    size_t i1, i2;
    uint32_t tag;
