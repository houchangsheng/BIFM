...
{
	unsigned int* hash = (unsigned int*)malloc(sizeof(uint256));
	hash[0] = 0xCAEE0535;
	hash[1] = 0x1023B4C8;
	hash[2] = 0x47B6BBB3;
	hash[3] = 0xC8B15368;
	...
	unsigned int* result = (unsigned int*)malloc(sizeof(uint256));
	unsigned int* swap;
	...
	free(result);  //If memory release is not performed, it will occupy a large amount of memory space.
	return hash;
}
