...
void print_bf(BLOOMFILTER* bf, int idx)
{
	int j;
	FILE* fp;
	if (idx == 0)
        {
		fp = fopen("Filter.sig", "wb");
        }
        else
        {
		fp = fopen("Filter.sig", "ab+");
	}
...
void readFileToBF(BLOOMFILTER* bf, int idx)
{
	int i;
	FILE* fp = fopen("Filter.sig", "rb");
	if (fp != 0)
	{
		fseek(fp, idx * BF_SIZE_IN_BYTES, SEEK_SET);
		...
		{
			fread(&bf->bytes_array[i], sizeof(bf->bytes_array[i]), 1, fp);
		}
		fclose(fp);
	}
}
